export class Rock extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, rocks) {
        ...
        let that = this;
        // Muss extra deklariert werden, damit es unten mit 
        // this.scene.events.off("update", this.updateFunction);
        // wieder deaktiviert deaktiviert werden kann.
        this.updateFunction = () => {
            that.onUpdate();
        };
    ...
    explode() {
        let explosion = this.scene.add.sprite(this.x, this.y, "spritesheet").play("explosion");
        explosion.scale = 2.0;
